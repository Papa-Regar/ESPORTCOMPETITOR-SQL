...
--SHOW PLAYER NAME WHERE SALARY GREATER THAN 100,000

SELECT player_name, salary
FROM players
WHERE salary > 100000

--WHAT IS THE TEAM NAME OF PLAYER ID=3

SELECT team_name
FROM teams,players
WHERE players.player_id=3 AND players.team_id=teams.team_id

--What is the total number of players in each team?

SELECT team_name, COUNT(players.player_id) AS Players
FROM teams, players
...
FROM players, teams
WHERE teams.team_id=2 AND teams.captain_id=players.player_id
...
SELECT country, AVG(players.salary)
FROM Players, Teams
WHERE Teams.country='USA' AND Players.team_id=Teams.team_id
GROUP BY Teams.country

--Which team win most matches?

 
SELECT TOP 1 team_name, MAX(win_counts) AS higest_win
FROM (
    SELECT Teams.team_name, COUNT(Matches.winner_id) AS win_counts 
    FROM Teams
